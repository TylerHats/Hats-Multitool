# Windows Update Module - Tyler Hatfield - v2.12
...
$latest = Find-PackageProvider -Name NuGet -AllVersions | Sort-Object Version -Descending | Select-Object -First 1
$installed = Get-PackageProvider -Name NuGet -ListAvailable -ErrorAction SilentlyContinue | Sort-Object Version -Descending | Select-Object -First 1
$needsInstall = -not $installed -or ($installed.Version -lt $latest.Version)
if ($needsInstall) {
    Install-PackageProvider -Name NuGet -RequiredVersion $latest.Version -Scope AllUsers -Force -Confirm:$false | Out-Null
}
Import-PackageProvider -Name NuGet -MinimumVersion $latest.Version -Force -ErrorAction SilentlyContinue | Out-Null
try {
...
# Set Download Mode
Import-Module DeliveryOptimization -WarningAction SilentlyContinue
Set-DODownloadMode -downloadMode Internet
Restart-Service -Name DoSvc -ErrorAction SilentlyContinue

<# Leaving out due to intermittent issues
